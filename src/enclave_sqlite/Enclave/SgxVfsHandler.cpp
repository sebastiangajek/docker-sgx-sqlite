...
std::string getSgxVfsName() {
	...
	struct Functions {
		static int xOpen(sqlite3_vfs*, const char *zName,
			...
			fileData->sgxData = sgx_fopen_auto_key(zName, "r+");

			if (fileData->sgxData == NULL) {
				fileData->sgxData = sgx_fopen_auto_key(zName, "w+");
			}
		...
		static int xWrite(sqlite3_file *fileBase, const void *buffer,
			...
			// fill database file with dummy content in case writer offset
			...
			if (resultSeek == -1) {
				int32_t error = sgx_ferror(fileData->sgxData);
				return SQLITE_IOERR_SEEK;
			}
